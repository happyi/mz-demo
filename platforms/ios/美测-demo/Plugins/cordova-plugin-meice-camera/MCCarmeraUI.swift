...
//    var luminance = [93,110,120,178,173]
    var step = 0
    let steps = ["左脸","右脸","正脸"]
    ...
    override func viewDidLoad() {
        ...
        self.imgView.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight)
...
//
//                self.prepareLight(index: index)
//                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now()+3) {
//                    self.camera.capture(forLight: index);
//                }
//            }
        }else{
...
//        self.current = index
        ...
        if self.imgView.viewWithTag(900) == nil{
            ...
            layerView.tag = 900
            layerView.frame = CGRect(x: 0, y: ScreenHeight-104, width: ScreenWidth, height: 104)
            // layerFillCode
            let layer = CALayer()
        ...
        if self.view.viewWithTag(tag) == nil{
            ...
            resetBtn.frame = CGRect(x:251, y: ScreenHeight-74, width: 111, height: 48)
        ...
        if self.view.viewWithTag(tag) == nil {
            if self.step == 0 || self.step == 1  {
                ...
                confirmBtn.frame = CGRect(x:406, y: ScreenHeight-74, width: 111, height: 48)
                confirmBtn.tag = tag
        ...
        if self.view.viewWithTag(tag) == nil {
            if self.step == 2 {
                ...
                confirmBtn.frame = CGRect(x:404, y: ScreenHeight-74, width: 111, height: 48)
                self.imgView.addSubview(confirmBtn)
            }
        }else{
            if self.step != 2
